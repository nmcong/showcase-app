...
# Simple deployment script for showcase-only (no database, no Keycloak)

set -e

echo "================================================"
echo "Simple Showcase Deployment"
echo "================================================"
...
cat > /tmp/app-deploy-simple.sh << 'REMOTESCRIPT'
#!/bin/bash
set -e

echo "================================================"
echo "Deploying Showcase Application"
echo "================================================"
...
# Create .env.local for Next.js (minimal config)
...
if [ -n "$VPS_PASSWORD" ]; then
    sshpass -p "$VPS_PASSWORD" ssh -p "$VPS_PORT" \
        "$VPS_USER@$VPS_HOST" "cat > /tmp/app-deploy-simple.sh" < /tmp/app-deploy-simple.sh
